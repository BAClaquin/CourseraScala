...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    ...
    def loop(chars: List[Char], list: List[(Char, Int)]): List[(Char, Int)]  = chars match {
      case head :: tail => loop(tail,insert(head, list))
      case Nil => list
    }
    ...
    def insert(char: Char,list: List[(Char,Int)]): List[(Char, Int)] = list match {
      case head :: tail => {
        ...
        else (head._1, head._2) :: insert(char, list.tail)
      }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    def loop(freqs: List[(Char, Int)], list: List[Leaf]): List[Leaf] = freqs match {
      case head :: tail => loop(freqs.tail, insert(head,list))
      case Nil => list
    }

    def insert(newLeaf: (Char, Int), list: List[Leaf]): List[Leaf] = list match {
      case head :: tail => {
        if (newLeaf._2 <= head.weight) Leaf(newLeaf._1,newLeaf._2) :: list
        else head :: insert(newLeaf, list.tail)
      }
      case Nil => List(Leaf(newLeaf._1, newLeaf._2))
    }
